...
frame_list=[tabs_frame,signup_mainframe,shop_scrollframe1,shop_widgets,shop_widgets_menu,supplies_frame,fruits_frame,dairy_frame,vegetable_frame,search_frame,profile_frame,orders_frame,settings_frame,supports_frame,notification_frame,cart_frame]
...
def login():
    global email_address, password_entry  # Declare as global to access in other functions
...
#CATEGORY TABS
    ...
    for x_pos in x_positions:
        product_label = ctk.CTkLabel(product_frame_menu, text='', height=190, width=140, fg_color='white', corner_radius=20)
...
def validate_email(event):
    ...
    # Regular expression to check the validity of the email including domain
    email_segex = r"^[\w\.-]+@[\w\.-]+\.[a-zA-Z]{2,}$"
    valid_length = 5 <= len(email) <= 254    
    if re.match(email_segex, email) and valid_length:
        feedback_label.configure(image=valid_image, compound='left', fg_color="#0c7075")
    else:
        feedback_label.configure(image=invalid_image, compound='left', fg_color="#0c7075")                        
...
#### INDIVIDUAL TABLE [USER DATA] ####

uniq=email_address.get()
...
# Create a table
cursor.execute('''CREATE TABLE IF NOT EXISTS uniq (
